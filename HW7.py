...
class LexerGUI:
    def __init__(self, root):
        ...
        self.input_Lbl = Label(root, text="Source Code Input:", borderwidth=2, relief="solid", 
                bg="black", fg="#0FFF50", highlightthickness=1, highlightbackground="#0FFF50")
        self.input_Lbl.grid(sticky=W, row=0, column = 0, padx=(20, 50), pady=(20, 0))
        self.output_Lbl = Label(root, text="Lexical Analyzed Result:", borderwidth=2, relief="solid", 
                bg="black", fg="#0FFF50", highlightthickness=1, highlightbackground="#0FFF50")
        self.output_Lbl.grid(sticky=W, row=0, column=1, pady=(20,0), padx=(9, 0))
        self.parse_Lbl = Label(root, text="Parse Tree:", borderwidth=2, relief="solid",
                bg="black", fg="#0FFF50", highlightthickness=1, highlightbackground="#0FFF50")
        self.parse_Lbl.grid(sticky=W, row=0, column=2, pady=(20,0), padx=(50, 0))
        self.process_Lbl = Label(root, text="Current Processing Line:\t\t\t\t", borderwidth=2, relief="solid", 
                bg="black", fg="#0FFF50", highlightthickness=1, highlightbackground="#0FFF50")
        self.process_Lbl.grid(sticky=W, row=2, column=0, padx=(20, 0))
        self.counter_Lbl = Label(root, text=str(self.counter), borderwidth=2, relief="solid", padx=20, 
                bg="black", fg="#0FFF50", highlightthickness=1, highlightbackground="#0FFF50")
        ...
        self.style.configure('Vertical.TScrollbar', troughcolor="black", background="#3F3F3F", 
        ...
        self.input_Box = Text(root, wrap=WORD, height=20, width=55, bg="black", fg="#0FFF50", 
                insertbackground="white", highlightbackground="#0FFF50", yscrollcommand=self.input_scroll_y.set)
        self.input_Box.grid(row=1, column=0, padx=(20, 50))
        self.output_Box = Text(root, wrap=WORD, height=20, width=55, bg="black", fg="#0FFF50", 
                insertbackground="white", highlightbackground="#0FFF50", yscrollcommand=self.output_scroll_y.set)
        self.output_Box.grid(row=1, column=1)
        self.parse_Box = Text(root, wrap=WORD, height=20, width=55, bg="black", fg="#0FFF50",
                insertbackground="white", highlightbackground="#0FFF50", yscrollcommand=self.parse_scroll_y.set)
        self.parse_Box.grid(row=1, column=2, padx=(50, 20))

        #Create and place Buttons
        self.next_line = Button(root, text="Next   Line", padx=20, bg="black", fg="#0FFF50", 
                highlightbackground="#0FFF50", command=self.nextLine)
        self.next_line.grid(sticky=E, row=3, column=0, padx=(20, 50), pady=(10, 20))
    ...
    def nextLine(self):
        ...
        if self.counter > 0:
            self.input_Box.tag_remove("start", f"{self.counter}.0", f"{self.counter}.end") #Remove previous highlight if there is one
            #self.parse_Box.delete("1.0", "end")

        input_txt = self.input_Box.get(f"1.0", END).split('\n') #Get a list of all input
        ...
        if self.counter < len(input_txt) - 1 and input_txt[0] != '':
            ...
            input_txt = self.input_Box.get(f"{self.counter}.0", f"{self.counter}.end") #Get the line of input

            #Add a highlight to the current line
            self.input_Box.tag_add("start", f"{self.counter}.0", f"{self.counter}.end")
            self.input_Box.tag_config("start", background="#ff0000")
        
            output_tokens = CutOneLineTokens(input_txt) #Get the token list from HW3 functions

            self.output_Box.insert(f"{self.counter}.0", f"{self.counter}. ") #Display the line number
            ...
            for i in range(len(output_tokens)):
                if i < len(output_tokens) - 1:
                    self.output_Box.insert(f"{self.counter}.{index}", f"{output_tokens[i]}, ")
                else:
                    self.output_Box.insert(f"{self.counter}.{index}", f"{output_tokens[i]}")
                index += len(output_tokens[i]) + 2 #Update starting point for insertion based on token length

            self.counter_Lbl.config(text=str(self.counter)) #Update counter
            self.output_Box.insert(f"{self.counter}.{index}", f"\n") #Add a new line for next input line

            parser(self.counter) #Parse the output of the lexer

            self.parse_Box.insert(END, f"{msg}") #Output the parse tree into the output box
...
def CutOneLineTokens(one_line):
    ...
    while one_line != "":
        ...
        token, tk_type = confirmPattern(one_line, choice) #Get the current token and type
...
def exp():
    ...
    if(inToken[0] == "identifier"):
        ...
        msg = "Error: exp expects identifier as the second element of the expression!\n\n"
        return
...
def comparison_exp():
    ...
    if inToken[1] == '>':
        ...
        msg = "Error: comparison_exp expects > after the identifier in the comparison_exp!\n\n"
...
def if_exp():
    ...
    if(inToken[1] == "if"):
        ...
        msg = "Error: if_exp expects keyword:if as the first element of the expression!\n\n"
        return
    ...
    if msg.find("Error") == -1: #Check for errors building tree
        ...
        if(inToken[1] == ')'):
            ...
            msg = "Error: if_exp expects ) as the fourth element of the expression!\n\n"
            return
...
def print_exp():
    ...
    if(inToken[1] == "print"):
        ...
        msg = "Error: print_exp expects keyword:print as the first element of the expression!\n\n"
        return
...
def string_exp():
    ...
    if(inToken[1] == '\"' or inToken[1] == '“' or inToken[1] == '\''):
        ...
        msg = "Error: string_exp expects \" as the first element of the expression!\n"
        return
    ...
    if inToken[0] == "str_literal":
        ...
        msg = "Error: string_exp expects str_literal as second element of the expression!\n\n"
    ...
    if inToken[1] == '\"' or inToken[1] == '”' or inToken[1] == '\'':
        ...
        msg = "Error: string_exp expects \" as the third element of the expression!\n\n"
        return
...
def parser(line):
    ...
    if (inToken[1] == ';' or inToken[1] == ':') and error == -1: #Done building tree
